...
<div key={props._key} className="flex flex-col md:flex-row">
  <div className="md:w-1/2 flex items-center justify-center aspect-video w-full max-h-screen xs:-mt-20">
    <div className="z-10 text-center">
      <h1 className="text-4xl font-bold xs:text-4xl sm:text-6xl">{title}</h1>
      <p className="mt-2 xs:text-2xl max-w-5xl p-4">{description}</p>
      <div className="mt-6 mb-2">
  ...
  <div className="md:w-1/2 relative">
    <Image {...image} className="w-full h-full object-cover" alt="" />
    <div className="absolute top-0 left-0 w-full h-full opacity-30 z-10"></div>
  </div>
</div>
      );
